...
exports.sendGem = catchAsyncErrors(async (req, res, next) => {
	...
	const lastPrice = prices[priceLength - 1];
	const gmc = numAmount / lastPrice.price;
	const gemRecipient = numAmount / lastPrice.price;

	// update sender balance
	sender.balance = sender.balance - numAmount;
	sender.gem_coin = sender.gem_coin - gmc;
	await sender.save();
	createTransaction(
		sender._id,
		'cashOut',
		numAmount,
		'Send GMC',
		`Send Gem Coin To ${recipient.name}`,
		lastPrice.price
	);
	...
	recipient.gem_coin = recipient.gem_coin + gemRecipient;
	await recipient.save();
	createTransaction(
		recipient._id,
		'cashIn',
		numAmount,
		'Receive GMC',
		`Receive Gem Coin From ${sender.name}`,
		lastPrice.price
	);
